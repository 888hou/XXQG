...
var dyNum = 2;//订阅 2
var commentText = ["广西人民欢迎您来到广西投资旅游参观！"];//评论内容，可自行修改，大于5个字便计分
...
ui.layout(
    <drawer id="drawer">
        <vertical>
            <appbar>
                <toolbar id="toolbar" title="gxylfm学习"/>
                <tabs id="tabs"/>
            </appbar>
            <viewpager id="viewpager">
                <frame>
                    ...
                    <vertical>
                        ...
                        <button text="-------独家VIP定制单独内容-------" style="Widget.AppCompat.Button.Borderless.Colored" w="*"/>
                        <button id="ksp" w="*" text="看视频" style="Widget.AppCompat.Button.Colored" />
                        ...
                        <button id="qita" w="*" text="一键其他，☟☟下面内容更劲爆" style="Widget.AppCompat.Button.Colored" />
                       
                        <button text="-------公告-------" style="Widget.AppCompat.Button.Borderless.Colored" w="*"/>
                        <button text="仅用于Autojs交流学习，禁止商用" style="Widget.AppCompat.Button.Borderless.Colored" w="auto"/>
                ...
                <frame>
                    <vertical>
                        ...
                        <text text="作者：@gxylfm  广西人民欢迎您来到广西投资旅游参观！" />
                    </vertical>
                </frame>
            </viewpager>
        </vertical>
        <vertical layout_gravity="left" bg="#ffffff" w="280">
            <img w="280" h="200" scaleType="fitXY" src="https://ftp.bmp.ovh/imgs/2021/06/0c412c0c300fdb56.jpg"/>
            <list id="menu">
...
ui.v100.setText(" 版本号：v1.0.0:2021.6.13  \n 1.修改自徐子越的2.5.1版本，向源码作者致敬。 \n 2.我增加了百灵视频观看，自定义UI布局功能 \n 3.删除了启动的自动更新网络题库，真的很烦这个。\n 4.更新题库到6月11日的最新版\n 5.我不是大神，也是小白，在学着看代码。");
...
ui.emitter.on("options_item_selected", (e, item)=>{
    switch(item.getTitle()){
        case "协议":
            ...
            break;
        case "警告":
            alert("警告", "仅Auto.js学习使用，禁止商用");
            break;
        case "说明":
            alert("使用说明",
                  "☭程序需要 悬浮窗 和 无障碍权限（设置→辅助功能→无障碍→本 APP）\n ☭程序工作原理为模拟点击，基于Auto.js框架+JavaScript脚本执行 \n ☭免责声明：本程序只供个人学习Auto.js使用，不得盈利传播，不得用于违法用途，否则造成的一切后果自负！"
            );
            break;
    }
    e.consumed = true;
...
//设置滑动页面的标题
ui.viewpager.setTitles(["任务", "个性设置", "题库", "更新说明", "作者信息"]);
//让滑动页面和标签栏联动
...
ui.webview.loadUrl("https://ftp.bmp.ovh/imgs/2021/06/3045962450748bc1.jpg");

ui.click_me.on("click", ()=>{
    toast("选择'gxylfm学习'开启无障碍");
    engines.execScript("选择'gxylfm学习'开启无障碍","auto.waitFor();console.show();console.hide();");
});
...
function article_timing(n, seconds) {
    ...
    randNum = parseInt(randNum); //惨痛的教训:这里必须要转成int类型。
 ...
 * @return: null
 */
function videoStudy_bailing(vCount, rTime) {
    h = device.height;//屏幕高
    w = device.width;//屏幕宽
    x = (w / 3) * 2;//横坐标2分之3处
    h1 = (h / 6) * 5;//纵坐标6分之5处
    ...
    delay(2);
    console.log("正在观看视频，您可以先去喝杯水！")
    delay(60);console.log("已经观看60秒，还剩300秒")
    delay(60);console.log("已经观看120秒，还剩240秒")
    delay(60);console.log("已经观看180秒，还剩180秒")
    delay(60);console.log("已经观看240秒，还剩120秒")
    delay(60);console.log("已经观看300秒，还剩60秒")
    delay(30);console.log("还有30秒，准备结束了")
    delay(20);console.log("最后倒数-10秒")
    delay(10);console.log("结束了，可以返回主页了")
    delay(3)
    back();console.log("返回主页！")
    id("home_bottom_tab_button_work").findOne().click();//点击主页正下方的"学习"按钮
    delay(5)
}
...
function CollectAndShare() {
    ...
    id("home_bottom_tab_button_work").findOne().click();//点击主页正下方的"学习"按钮
    delay(2);
    click((w/2)+random()*10,h/4);//坐标点击一个位置
    while (!textContains("欢迎发表你的观点").exists())//如果没有找到评论框则认为没有进入文章界面，一直等待
    {
        delay(1);
    ...
    }).findOnce();//var shareIcon = classNameContains("ImageView").depth(10).findOnce(1);//右下角分享按钮
...
function Comment() {
    ...
    console.info("评论删除成功!");
    delay(1);
    var shareIcon = classNameContains("ImageView").depth(10).bounds(616,1356,704,1404).findOnce();//右下角分享按钮
    shareIcon.click();//点击分享
    while (!textContains("分享到学习强国").exists());//等待弹出分享选项界面
    delay(1);
    click("分享到学习强国");
...
function localChannel() {
    ...
    console.log("点击省份");
    delay(1);
    var shareIcon = classNameContains("TextView").depth(17).bounds(397,175,459,213).findOnce()
    shareIcon.click();//点击省份
    log("点击省份")
    delay(2);
    var shareIcon = classNameContains("TextView").depth(27)(bounds(397,175,459,213)).findOnce()
    shareIcon.click();//点击本地平台
    log("点击本地平台")
    delay(2);
    back();
    videoStudy_bailing()
